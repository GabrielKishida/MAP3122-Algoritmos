...
def switchLines(matrix,line1,line2):
    auxvector = np.array(matrix[line1])
...
def pivotalElimGauss(matrix):
    n = len(matrix)
    for i in range(n):
        #Before we start the method, we need to apply pivotal condensation
        biggestPivot = i
        for k in range(i,n) :
            if (abs(matrix[biggestPivot][i]) < abs(matrix[k][i])) :
                biggestPivot = k
        if (biggestPivot != i) :
            switchLines(matrix,i,biggestPivot)
        #Firstly, this method "eliminates" the values in the lower triangle of the matrix.
        for j in range(i+1,n):
            lineElimination(matrix,i,j)
    if (matrix[n-1][n-1] == 0): return "System not solvable"
